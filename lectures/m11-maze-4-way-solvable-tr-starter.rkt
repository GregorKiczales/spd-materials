...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname m11-maze-4-way-solvable-tr-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(@assignment lectures/m11-maze-4-way-solvable-tr)
(@cwl ???) ;replace ??? with your cwl

(@problem 1)
;;
;; Revise this program so that for mazes like M7 it only works on each
;; position once.
;; Use tail recursion to maintain a visited accumulator.
;;
...
(define (solve m)
  (local [(define R (sqrt (length m)))
          ...
          #;#;
          ;; ---
          ;; original, structural recursion, no path or visited
