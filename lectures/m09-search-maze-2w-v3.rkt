...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname m09-search-maze-2w-v3) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
(define (solvable? m)           
  ...
  (local [(define (fn-for-pos p)
          ...
          ;; !!!
          ;; produces true if p is the position at the end of the maze
          (define (solved? p) false)


          
          ;; (@htdf valid-next-positions)
          ;; (@signature Position -> (listof Position)
          ;; !!!
          ;; produces positions reachable from p, moving only right or down
          (define (valid-next-positions p) empty)]
