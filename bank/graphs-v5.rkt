...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname graphs-gen-v5) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
(require spd/tags)
...
;; room. For example:
;; 
;;   (reachable? RF "F") produces true
;;   (reachable? RF "A") produces false
;;   (reachable? RA "E") produces true
;;   (reachable? RA "X") produces false
;;  
;; Note that (reachable? HF "A") would produce false because it is not possible
...
(check-expect (reachable? RF "F") true)
(check-expect (reachable? RF "A") false)
(check-expect (reachable? RA "E") true)
(check-expect (reachable? RA "X") false)

;; non-tail recursive version
(@template-origin genrec Room (listof String) accumulator try-catch encapsulated)
#;
  ...
  (local [(define (fn-for-room r path) 
          ...
          (define (fn-for-lor lor path)
            (cond [(empty? lor) false]
                  [else
                   (local [(define try (fn-for-room (lookup-room (first lor))
                                                    path))]
                     (if (not (false? try))
                         try
                         (fn-for-lor (rest lor) path)))]))]
