...
;; fs-v3.rkt (complete data-definition, sum-data function and all-names
;;            function)
...
(@htdf sum-data--element sum-data--loe)
(@signature Element -> Integer)
(@signature ListOfElement -> Integer)
...
(@signature Element       -> ListOfString)
(@signature ListOfElement -> ListOfString)
...
(check-expect (all-names--loe (list D4 D5))
              (append (list "D4" "F1" "F2") (list "D5" "F3")))
(check-expect (all-names--element D6)
...
(@template-origin ListOfElement try-catch)
